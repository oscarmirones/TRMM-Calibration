...
```
```{r}
idx <- which(cyclones$cyclones_generation_centers > 0)
cyclones$cyclones_generation_centers[idx] <- 1
```
...
levelplot(Z ~ Y*X, data=data,  xlab = "", ylab = "WT", scales=list(x=list(at=seq(1,12,1), labels = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sept","Oct","Nov","Dec"))),
...
```{r}
scale.1.100 <- function(x){
  return(((x-min(cyc.data$Z))/(max(cyc.data$Z)))*100)
}
```

```{r}
#values <- scale.1.100(cyc.data$Z)

#values[which(values < 1)] <- 1
#bub <- data.frame(rep(1,length(cyc.data$Z)),values)
#colnames(bub) <- c("x","y")
```

```{r}
#ggplot(bub, aes( x = x, y = y, size = y)) + geom_point(fill = 'blue', colour = 'blue')+theme_bw()+scale_size_area(max_size = 10, limits = c(1,100))+theme(legend.position = "top", legend.direction = "horizontal") + labs(size = "Dot size")
```
...
```{r}
levelplot(Z ~ Y*X, data=data,  xlab = "", ylab = "WT", scales=list(x=list(at=seq(1,12,1), labels = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sept","Oct","Nov","Dec"))),
          main="", col.regions = rev(hcl.colors(100,palette = 'GnBu')), colorkey = list(width = 0.75))+
  layer(sp.points(cyc.data, pch = 19, cex = scale.1.100(cyc.data$Z)/25, col = 'orange'))+
